      ...
      <div class="slide-tabs-nav" ref="tabsNav">
        <template v-for="(item, index) in navItems" :key="`${item.name}-${languageKey}`">
          <router-link
            ...
            :class="{
              active:
                ...
                (route && route.meta && route.meta.activeNav === item.name),
            }"
          ...
          >
            ...
            <span v-if="item.name === 'Invite' && showInviteBadge" class="badge-dot">{{
              $t('menu.commission')
            }}</span>

            <span v-if="item.name === 'Shop' && showShopBadge" class="badge-dot">{{
              $t('menu.hotSale')
            }}</span>
          </router-link>
        </template>
...
<script>
  ...
  import { IconServer, IconChartBar, IconHeadset } from '@tabler/icons-vue';
    ...
    setup() {
      ...
      const checkAndFixSliderPosition = () => {
        ...
        try {
          ...
          if (
            ...
            activeIndex >= 0 &&
            activeIndex < navItemElements.length
          ) {
      ...
      const onLanguageChanged = () => {
        ...
        sliderState.transition =
          'width 0.6s cubic-bezier(0.25, 0.1, 0.25, 1), transform 0.6s cubic-bezier(0.25, 0.1, 0.25, 1), opacity 0.3s ease';
