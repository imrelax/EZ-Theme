    ...
    <div v-if="show" class="node-detail-modal-overlay" @click.self="close">
      <div class="node-detail-modal-container">
        ...
        <transition :name="slideDirection" mode="out-in">
          ...
          <div v-if="currentPage === 'info'" key="info" class="node-detail-modal-body">
            ...
            <div class="node-info-section">
              ...
              <div class="info-row">
                ...
                <span class="info-value" :class="{ 'high-rate': parseFloat(node.rate) != 1 }"
                  >x{{ node.rate }}</span
                >
              </div>
              ...
              <template v-if="node.type === 'trojan'">
                <div class="info-row">
                  ...
                  <span class="info-value">{{
                    node.allow_insecure ? $t('nodes.allowInsecure') : $t('nodes.secure')
                  }}</span>
                </div>
              </template>
              ...
              <template v-if="node.type === 'vmess'">
                <div class="info-row">
                  ...
                  <span class="info-value">{{
                    node.tls === 1 ? $t('common.on') : $t('common.off')
                  }}</span>
                </div>
              </template>
            </div>
          ...
          <div
            v-else-if="currentPage === 'subscribe'"
            key="subscribe"
            class="node-detail-modal-body"
          >
            <!-- 快速连接信息切换 -->
      ...
      case 'vless': {
        ...
        if (props.node.tls) {
          ...
          if (props.node.tls === 2 && props.node.tls_settings) {
            ...
            if (
              props.node.tls_settings.server_port &&
              props.node.tls_settings.server_port !== props.node.port.toString()
            ) {
