...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      const preloadImages = ref(['/images/logo.png']);
      ...
      const preloadExternalResources = () => {
        ...
        resources.forEach((resource) => {
          if (!preloadManager.isResourceLoaded(resource.href)) {
            const link = preloadManager.createPreloadLink(
              resource.href,
              resource.as,
              resource.type
            );
      ...
      const enqueueComponents = (components) => {
        ...
        const newComponents = sortedComponents.filter(
          (comp) => !preloadManager.isComponentLoaded(comp.name)
        );
        ...
        (newPath, oldPath) => {
          if (newPath !== oldPath && newPath in componentsConfig.route) {
            if (
              ...
              newPath !== '/customer-service' &&
              !componentsConfig.route[newPath].some((comp) => comp.name === 'CustomerService')
            ) {
